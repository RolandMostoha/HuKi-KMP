...
| Status | Scope      | Bug                                                                                                                                                                                                                                                                                                                                              |
|--------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `[x]`  | Search     | Bug: iOS. Search fails a lot. Not a rate limit — `search_failed` was a catch-all: LocationIQ answers "no results" with a 404, and Darwin's `DarwinHttpRequestException` never matched the JVM-only offline check, so every offline/timeout failure fell through to UNKNOWN + a Crashlytics non-fatal. Each `NetworkError` now has its own event. |
